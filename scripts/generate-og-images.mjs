...
// ─── Generate generic preset share OG image ─────────────────────────

async function generatePresetOG() {
  const svg = `<svg width="${OG_WIDTH}" height="${OG_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect width="${OG_WIDTH}" height="${OG_HEIGHT}" fill="url(#pbg)"/>
    <text x="${OG_WIDTH / 2}" y="200" font-family="Arial, Helvetica, sans-serif" font-size="52" font-weight="bold" fill="white" text-anchor="middle">See Through My Eyes</text>
    <text x="${OG_WIDTH / 2}" y="270" font-family="Arial, Helvetica, sans-serif" font-size="26" fill="#94a3b8" text-anchor="middle">Custom Vision Simulation</text>
    <text x="${OG_WIDTH / 2}" y="340" font-family="Arial, Helvetica, sans-serif" font-size="22" fill="#60a5fa" text-anchor="middle">Experience what vision looks like with various conditions</text>
    <text x="${OG_WIDTH / 2}" y="400" font-family="Arial, Helvetica, sans-serif" font-size="20" fill="#94a3b8" text-anchor="middle">Try the free interactive simulator</text>
    <rect x="0" y="${OG_HEIGHT - 50}" width="${OG_WIDTH}" height="50" fill="#1e3a8a"/>
    <text x="${OG_WIDTH / 2}" y="${OG_HEIGHT - 18}" font-family="Arial, Helvetica, sans-serif" font-size="20" font-weight="bold" fill="white" text-anchor="middle">theblind.spot</text>
  </svg>`;
...
async function main() {
  ...
  // Generate generic preset share OG image
  const presetOG = await generatePresetOG();
  writeFileSync(join(OG_DIR, 'preset-share.png'), presetOG);
  // eslint-disable-next-line no-console
  console.log('  Generated og/preset-share.png');
