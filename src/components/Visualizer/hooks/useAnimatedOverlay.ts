 ...
 */
export const ANIMATED_EFFECTS = ['visualAura', 'visualAuraLeft', 'visualAuraRight', 'hallucinations', 'blueFieldPhenomena', 'persistentPositiveVisualPhenomenon', 'palinopsia', 'starbursting', 'christineFluctuatingVision', 'sugarRetinalDetachmentComplete', 'sugarPeripheralFlashes', 'stephenKeratoconusComplete'];
 ...
 */
function generateChristineFluctuatingOverlay(
  intensity: number,
  now: number
): React.CSSProperties {
  const time = now / 1000;
 ...
 */
function generateSugarRetinalDetachmentOverlay(
  intensity: number,
  now: number
): React.CSSProperties {
  const time = now / 1000;
 ...
 */
function generateStephenKeratoconusOverlay(
  intensity: number,
  now: number
): React.CSSProperties {
  const time = now / 1000;
...
/**
 * Hook that generates animated overlay styles for visual aura, hallucinations, blue field, PPVP, palinopsia, starbursting, Christine Ha, Sugar Ray Leonard, and Stephen Curry effects
 */
