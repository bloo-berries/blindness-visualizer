...
export const tophBeifongEffects: VisualEffect[] = [
  effect('tophSeismicSenseComplete', 'Seismic Sense (Toph Beifong)', 'Complete seismic sense simulation. The world renders as a topographic wireframe in phosphor green - like sonar or LiDAR. Only ground-coupled surfaces are visible; airborne objects are invisible. Solid rock/metal renders crisp; sand is blurry (her canonical weakness). Living beings pulse with heartbeat rhythm. Motion creates ripple waves. "Vision" fades in stillness and refreshes with each vibration event.'),
  effect('tophWireframeVision', 'Wireframe Terrain (Toph)', 'Topographic wireframe rendering - the world appears as a monochromatic height-field mesh. No color, no texture, no optical light/shadow. Dark background with geometry in pale phosphor glow.'),
  effect('tophVibrationRipples', 'Vibration Ripples (Toph)', 'Motion ripples emanating from contact points - concentric wave distortion rings like raindrops on a still pond. Intensity scales with force of impact.'),
  effect('tophHeartbeatDetection', 'Heartbeat Detection (Toph)', 'Living beings render with subtle pulsing amplitude modulation synced to heartbeat (~1-1.5 Hz). Toph canonically detects heart rate through vibrations.'),
  effect('tophSandWeakness', 'Sand Blindness (Toph)', 'Simulates Toph\'s canonical weakness on sand - poor vibration transmission creates blurry, low-confidence, unreliable perception. The "image" becomes fuzzy and uncertain.'),
];
